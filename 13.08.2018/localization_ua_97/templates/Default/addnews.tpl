...
$(function(){
	$('#category').chosen({allow_single_deselect:true, no_results_text: 'Нічого не знайдено'});
});
</script>
<div class="pheading"><h2>Додати новину</h2></div>
<div class="baseform">	
	...
	[urltag]
		<tr>
			<td class="label">URL статті:</td>
			<td><input type="text" name="alt_name" value="{alt-name}" maxlength="150" class="f_input" /></td>
		</tr>
	[/urltag]
		<tr>
			<td class="label">
				Категорія:<span class="impot">*</span>
			</td>
			<td>{category}</td>
		</tr>
		<tr>
			<td colspan="2">
				<b>Короткий зміст: <span class="impot">*</span></b> (Обов'язково)
				<div>
					[not-wysywyg]
					<div>{bbcode}</div>
		...
		<tr>
			<td colspan="2">
				<b>Повний зміст:</b> (Не обов'язково)
				<div>
					[not-wysywyg]
					<div>{bbcode}</div>
		...
		{xfields}
		<tr>
			<td class="label">Ключові слова для хмаринки тегів:</td>
	...
	<div class="fieldsubmit">
		<button name="add" class="fbutton" type="submit"><span>Надіслати</span></button>
		<button name="nview" onclick="preview()" class="fbutton" type="submit"><span>Перегляд</span></button>
	</div>
</div>